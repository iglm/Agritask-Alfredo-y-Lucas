...
const OptimizationSuggestionSchema = z.object({
    explanation: z.string().describe("A clear, human-readable description of the optimization suggestion. For example, 'Reassign task X from Juan to Maria to balance workload.'"),
...
const ResourceOptimizerInputSchema = z.object({
  jsonData: z.string().describe("A JSON string containing arrays of 'tasks' (for the upcoming period) and 'staff' from the farm management system. Tasks may include 'downtimeMinutes'."),
...
const optimizerPrompt = ai.definePrompt({
  ...
  prompt: `
    ...
    3.  Your mission is to analyze workload and downtime to propose structured, executable actions in JSON format.
    4.  If workload is balanced, you MUST return an object with an empty "suggestions" array.
    
    DATA TO ANALYZE:
    -   JSON data containing 'staff' and scheduled 'tasks'.
    ...
    -   **Workload Analysis (Action: \`reassignTask\`):**
        ...
        -   If a significant imbalance is found (e.g., one worker has > 1.5x the load of another available worker), generate a \`reassignTask\` action with 'Baja' severity.
